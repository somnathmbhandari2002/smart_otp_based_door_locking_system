...
async def send_otp(data: SendOTP):
    ...
    print(f"📨 Received OTP request: {data.otp} for {data.mode}")
    
    # Clear previous OTPs
    delete_result = otp_collection.delete_many({})
    print(f"🗑️ Cleared {delete_result.deleted_count} previous OTPs")
    ...
    print(f"💾 OTP saved to database: {data.otp}")
    ...
    <html>
        <body style="font-family: Arial, sans-serif; padding: 20px;">
            <h2 style="color: #333;">🔐 Smart Door OTP</h2>
    ...
    try:
        message = MessageSchema(
            subject=f"🔐 Your Smart Door OTP for {data.mode.upper()}",
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 Starting Smart Door OTP Server...")
    uvicorn.run(
        "main:app",
